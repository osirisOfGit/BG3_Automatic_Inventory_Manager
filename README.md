...
| SORT_ITEMS_ON_LOAD | `1` if you want to execute items when you load a campaign that hasn't had its items procesed by AIM yet - once a campaign has been processed and saved, a PersistentVar will be used to prevent re-processing on every load afterwards<br/>`0` if you just want it to happen when picking up an item | 1 |
